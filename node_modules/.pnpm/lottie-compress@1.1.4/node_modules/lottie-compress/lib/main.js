...
class LottieCompress {
    constructor(lottieJson, options) {
        let _lottieJson;
        ...
        this.options = Object.assign({ quality: [0.75, 0.9] }, options);
    }
    /**
     * execute
     */
    ...
    async miniBase64() {
        const assetsPromise = this.lottieJson.assets.map(async (asset) => {
            ...
            if (!asset.u && !asset.p
                || quality && quality[0] === 1 && quality[1] === 1) {
                return asset;
            }
            let imageData = null;
            let extname = 'png';
            ...
            if (this.options.tinypngKey) {
                tinify_1.default.key = this.options.tinypngKey;
                const source = tinify_1.default.fromBuffer(imageData);
                newBuf = await source.toBuffer();
            }
            else {
                newBuf = await imagemin.buffer(imageData, {
                    plugins: [
                        imagemin_pngquant_1.default({ quality }),
                    ],
                });
            }
            return Object.assign(Object.assign({}, asset), { e: 1, u: '', p: 'data:image/' + extname + ';base64,' + newBuf.toString('base64') });
        });
     ...
     */
    async tinybase64(stream, fileds) {
        ...
        const newBuf = await imagemin.buffer(imageData, {
            plugins: [
                imagemin_pngquant_1.default({ quality: this.options.quality }),
            ],
        });
        return newBuf;
    }
    async getJsonStream(stream) {
    ...
    async getStream(stream) {
        return await new Promise((resolve) => {
            let data = ''; // 创建一个buffer用于存储读取完的信息
            const buffers = []; // 创建一个数组，用于存储每一个流读取的信息
     ...
     * @return {object} 结果
     */
    _resetAttr(obj, attrNames, value) {
        const innerLoops = (innerObj) => {
            const list = this._loops(innerObj);
            (list || []).forEach(attr => {
                if (attrNames.indexOf(attr) !== -1) {
     ...
     * @return {object} returnobj lottie.json
     */
    attrEqualZip(obj, attrNames) {
        if (!(typeof attrNames === 'string' && attrNames.split(':')[1])) {
            return obj;
        }
...
exports.default = LottieCompress;
